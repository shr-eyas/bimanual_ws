...
bool move(const franka::Gripper& gripper, const MoveGoalConstPtr& goal) {
  double speed = goal->speed > 0.0 ? goal->speed : 0.05; // Use goal speed or a fallback
  double target_width = 0.0; // Force FULLY CLOSE
...
// bool grasp(franka::Gripper& gripper, const franka_gripper::GraspGoalConstPtr& goal) {
//   double force = goal->force > 0 ? goal->force : 20.0; // Default force if none provided
//   double target_width = 0.0;  // Fully close
//   double epsilon_inner = 0.01; // Allow some compression, e.g., 2mm inward
//   double epsilon_outer = 0.01; // Allow some tolerance outward too

//   bool success = gripper.grasp(target_width, goal->speed, force, epsilon_inner, epsilon_outer);
//   return success;
// }
